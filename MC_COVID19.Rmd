...
Die Karte soll so interaktiv gestaltet werden, dass alle Daten angezeigt und ausgewählt werden können.
```{r}
# # die Grundkarte bleibt dieselbe
# fig_all <- plot_geo(df, locationmode = 'country names') %>% 
#   add_trace(
#     z = ~Cases_per_10000, text = ~Country_Region,
#     locations = ~Country_Region,
#     colors = 'Reds',
...
#     )
#   ) %>% 
#   # Ergänzung für die Animation des Datums
#   animation_slider(
#     currentvalue = list(prefix = "Datum: "),
#     len = 0.9,
#     xanchor = "left",
#     yanchor = "top"
#   ) %>% 
...
#   ) 
# 
# # Daten für die Animation hinzufügen
...
#     text = ~Country_Region, 
#     locations = ~Country_Code, 
#     color = ~Cases_per_10000, 
#     colors = 'Reds',
#     marker = list(line = list(color = 'rgb(180,180,180)', width = 0.5))
#   ) %>%
#   colorbar(title = 'Fälle pro 10.000 Personen') %>%
#   layout(
#     title = 'COVID-19 Fallzahlen pro 10.000 Personen',
#     geo = list(
#       showframe = FALSE, 
...
#   )
# 
# # Fügen Sie die Daten für die Animation hinzu
# dates <- unique(df$Date)
# for(date in dates) {
#   day_data <- df %>% filter(Date == date)
#   fig <- fig %>% add_trace(
#     z = ~Cases_per_10000, 
#     locations = ~Country_Code,
#     frame = ~as.character(Date)
...
# Angenommen, Ihr DataFrame 'df' enthält die Spalten 'Date' (im Format 'yyyy-mm-dd'), 
# 'Country_Region', 'Country_Code' (ISO 3166-1 alpha-3 country codes), und 'Cases_per_10000'.

# Stellen Sie sicher, dass das Datum im richtigen Format vorliegt
df$Date <- as.Date(df$Date, format = "%Y-%m-%d")

# Erstellen Sie die Basis für die Choroplethenkarte
fig <- plot_ly(df, locations = ~Country_Code, z = ~Cases_per_10000, 
               text = ~Country_Region, colors = "Reds", type = 'choropleth', 
               hoverinfo = "text+z+location", marker = list(line = list(color = 'rgb(255,255,255)', width = 2)))

# Fügen Sie die Animation hinzu
fig <- fig %>% animation_opts(frame = 100, redraw = TRUE) %>%
  animation_slider(currentvalue = list(prefix = "Datum: "), 
                   steps = list(list(method = "animate", args = list(list("frame"), list(mode = "immediate", frame = 100, redraw = TRUE)))))


# Fügen Sie Frames für jedes Datum hinzu
dates <- unique(df$Date)
for (date in dates) {
  day_data <- df %>% filter(Date == date)
  fig <- fig %>% add_frame(data = day_data, name = ~as.character(date))
}
